...
# @ray.remote
def apply_dr(input_file, output_folder, dataset_name='MNIST', dr_name='PCA',
             perplexity=None, n_neighbors=None, min_dist=None, max_samples=10000):
    fn = "{dataset_name}{dr_name}{perp}{neigh}{mindist}".format(dataset_name = dataset_name,
                                                                dr_name = "_" + dr_name,
                                                                perp = "_p" + str(
                                                                    perplexity) if perplexity is not None else "",
                                                                neigh = "_n" + str(
                                                                    n_neighbors) if n_neighbors is not None else "",
                                                                mindist = "_d" + str(
    ...
    print(('---------Startings: {} - {}-----------'.format(dataset_name, fn)))
    ...
    if df.shape[0] > max_samples:
        X_train, features, y_train, labels = train_test_split(X, y, test_size = max_samples, random_state = 42,
                                                              stratify = y)
    else:
        features = X
        labels = y
